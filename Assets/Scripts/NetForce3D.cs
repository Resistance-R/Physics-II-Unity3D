...
public class NetForce3D : MonoBehaviour
{
    private float XThrust = 3.0f;
    private float YThrust = 4.0f;
    private float ZThrust = 5.0f;
    ...
    void Start()
    {
        ...
        myRigid.AddForce(transform.forward * ZThrust, ForceMode.Impulse);
    }
    ...
    void Update()
    {
        ...
        float theoreticalValue = Mathf.Sqrt(Mathf.Pow(XThrust, 2) + Mathf.Pow(YThrust, 2) + Mathf.Pow(ZThrust, 2));
